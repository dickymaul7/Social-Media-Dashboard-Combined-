...
export default function AuthGuard({children}:{children:React.ReactNode}){
 ...
 useEffect(()=>{const url=process.env.NEXT_PUBLIC_SUPABASE_URL,key=process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY;if(!url||!key){setReady(true);return}let valid=false;try{const raw=localStorage.getItem(SESSION_KEY);if(raw){const session=JSON.parse(raw);valid=Boolean(session?.access_token&&Number(session?.expires_at||0)>Date.now()/1000)}}catch{}if(!valid){if(pathname!=="/login")router.replace(`/login?next=${encodeURIComponent(pathname)}`);return}setReady(true)},[pathname,router]);
