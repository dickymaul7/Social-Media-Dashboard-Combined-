...
import {hydrateSharedTasks,loadTasks,loadUsers,upsertTask,type WorkspaceTask,type WorkspaceUser} from "@/lib/workspace-store";
...
export default function CalendarTaskAssignment(){
 const[target,setTarget]=useState<HTMLElement|null>(null);const[selected,setSelected]=useState<SelectedContent|null>(null);const[users,setUsers]=useState<WorkspaceUser[]>([]);const[assignee,setAssignee]=useState("");const[dueDate,setDueDate]=useState("");const[priority,setPriority]=useState<WorkspaceTask["priority"]>("medium");const[message,setMessage]=useState("");
 ...
 useEffect(()=>{const refresh=()=>setUsers(loadUsers().filter(user=>user.status!=="inactive"));refresh();window.addEventListener("proxsis-workspace:updated",refresh as EventListener);return()=>window.removeEventListener("proxsis-workspace:updated",refresh as EventListener)},[]);
 useEffect(()=>{const click=(event:MouseEvent)=>{const card=(event.target as HTMLElement|null)?.closest<HTMLElement>(".calendar-card");if(!card)return;const content=detectSelected(card);setSelected(content);setMessage("");if(!content){setAssignee("");setDueDate("");return}const existing=loadTasks().find(task=>task.brief_id===content.briefId&&(task.channel===content.channel||(!task.channel&&content.channel==="social")));setAssignee(existing?.assignee||"");setDueDate(existing?.due_date||content.scheduledFor||"");setPriority(existing?.priority||"medium")};document.addEventListener("click",click);return()=>document.removeEventListener("click",click)},[]);
