...
export default function ContentGeneratorPage() {
  ...
  async function generate(){
    ...
    try{
      ...
      saveCampaign(bundle);
      setHistory(loadCampaignsForBrand(activeBrand.id).slice(0,8));
  ...
  return <div className="generator-page">
    ...
    <div className="generator-header"><div><p className="eyebrow">BRIEF STUDIO · LIVE RESEARCH</p><h1>Quick Brief</h1><p className="muted">5 input utama → Topic Lock → live research → 5 case-led Story Angles.</p></div><div className={hasIntelligence?"intel-badge":"intel-badge missing"}><Brain size={16}/>{hasIntelligence?`Brand Intelligence ${source}`:"Brand Intelligence missing"}</div></div>
    ...
    <div className="generator-grid">
      <section className="panel form-panel"><div className="panel-head"><div><h2>Campaign Brief</h2><p>Draft tersimpan otomatis per active brand.</p></div><button className="ghost clear-draft" onClick={clearDraft}>Kosongkan</button></div>
        <label>1. Topic / Program <span className="topic-lock">Topic Lock</span><input value={topic} onChange={e=>setTopic(e.target.value)} placeholder="Contoh: AI untuk Human Capital"/></label>
        ...
        <div className="form-row"><label>5. Preferred Format<select value={format} onChange={e=>setFormat(e.target.value as Format)}><option value="auto">Auto</option><option value="carousel">Carousel</option><option value="reels">Reels</option><option value="single_post">Single Post</option></select></label><label>Research mode<div className="research-mode"><Search size={14}/> DeepSeek + Tavily</div></label></div>
        ...
        <button className="primary generate" onClick={generate} disabled={!hasIntelligence||loading}><Sparkles size={16}/>{loading?"Researching cases & validating topic fit...":"Generate 5 Story Angles"}<ArrowRight size={16}/></button>
      </section>
      <aside className="panel intel-panel"><div className="panel-head"><div><h2>Active Brand Intelligence</h2><p>{activeBrand.name} · injected automatically</p></div></div><div className="intel-box">{buildBrandContext(intelligence).split("\n").map(x=><p key={x}>{x}</p>)}</div><div className="research-note"><strong>Topic Lock aktif</strong><p>Brand Intelligence hanya menjadi guardrail. AI tidak boleh mengganti topik user dengan expertise brand lain. Case dengan topic relevance atau campaign alignment di bawah 7/10 ditolak.</p></div></aside>
    </div>
