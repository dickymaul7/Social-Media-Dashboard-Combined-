...
export async function POST(request:Request){
  try{
    ...
    const queryPlan=await createStructuredJson<QueryPlan>({schema:queryPlanSchema as unknown as Record<string,unknown>,system:"Kamu adalah search strategist untuk riset case study B2B. Tugasmu hanya membuat query pencarian presisi.",user:`TANGGAL: ${today}\n\nTOPIC LOCK — HARD CONSTRAINT:\n${input.topic}\n\nQUICK CAMPAIGN BRIEF:\n${compactJson(input)}\n\nBuat tepat 4 query web untuk menemukan kasus perusahaan/organisasi nyata yang LANGSUNG relevan dengan TOPIC LOCK. Brand Intelligence hanya boleh membantu konteks bisnis, audience, dan brand fit; tidak boleh mengganti topik user. Setiap query wajib memuat topik utama atau sinonim/konsep langsungnya. Prioritaskan: (1) recent incident/tension, (2) official/regulator/company evidence, (3) implementation/turnaround/successful response, (4) benchmark case kuat. Query harus mencari nama perusahaan, kejadian konkret, konsekuensi, keputusan, dan mechanism.`,temperature:0.2});
    ...
    const synthesis=await createStructuredJson<Synthesis>({schema:angleSynthesisSchema as unknown as Record<string,unknown>,system:"Kamu adalah gabungan senior B2B researcher, content strategist, dan executive storyteller. Semua output human-facing wajib Bahasa Indonesia. Tolak konten generik.",user:`EDITORIAL KNOWLEDGE BASE:\n${knowledge}\n\nTOPIC LOCK — USER REQUEST, HARD CONSTRAINT:\n${input.topic}\n\nQUICK INPUT:\n${compactJson(input)}\n\nACTIVE BRAND INTELLIGENCE — gunakan untuk HOW/guardrail, bukan untuk mengganti WHAT/topik:\n${compactJson(input.brandIntelligence)}\n\nPRIORITAS WAJIB:\n1. TOPIC LOCK dari user.\n2. Target audience + objective user.\n3. Evidence dari live web sources.\n4. Brand Intelligence sebagai positioning/tone/POV/capability guardrail.\n5. Editorial knowledge sebagai storytelling method.\nBrand Intelligence TIDAK BOLEH menggeser topik ke expertise lain.\n\nLIVE WEB SOURCES — fakta kasus hanya boleh berasal dari katalog ini:\n${sourceCatalog}\n\nTUGAS:\n1. Bentuk brand profile editorial yang konservatif.\n2. Turunkan campaign logic yang tetap menjawab TOPIC LOCK.\n3. Pilih 3-4 kasus nyata dengan tension + mechanism kuat dan idealnya >=2 source refs.\n4. Buat tepat 5 content angles case-led, non-generic, dan tetap berada di TOPIC LOCK.\n\nTOPIC RELEVANCE SCORING — WAJIB:\n- topic_relevance_score 9-10 = kasus secara langsung membahas/menerapkan/mengalami topik.\n- 7-8 = sangat dekat dan mekanismenya menjelaskan topik tanpa lompatan besar.\n- 0-6 = terlalu jauh / hanya cocok karena brand expertise / butuh lompatan interpretasi; dianggap tidak layak.\n- campaign_alignment_score menilai kesesuaian terhadap kombinasi TOPIC LOCK + objective + audience.\nJangan menaikkan skor hanya karena brand_fit tinggi.\n\nATURAN: wajib Case/Evidence → Tension → Mechanism → Insight → Brand POV. Jangan membuat angka, quote, motive, legal finding, atau sebab-akibat tanpa dukungan sumber. Audience: ${input.audience}. ${formatLabel(input.preferredFormat)} Extra context: ${input.extraContext||"Tidak ada."}`,temperature:0.25});
    ...
    if(eligible.length<2)return errorJson(`AI belum menemukan minimal dua kasus yang cukup relevan dengan topik “${input.topic}”. Brand context tidak boleh menggantikan topik. Perjelas topik atau generate ulang.`,502);
