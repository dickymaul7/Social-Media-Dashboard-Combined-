...
export function saveCampaign(bundle:CampaignBundle){
  if(typeof window==="undefined")return;
  window.localStorage.setItem(campaignKey(bundle.campaign.id),JSON.stringify(bundle));
  const ids=loadCampaignIds();
  const next=[bundle.campaign.id,...ids.filter(id=>id!==bundle.campaign.id)].slice(0,100);
  writeIds(campaignIndexKey,next);
}
...
export function saveBrief(brief:BriefRecord){if(typeof window==="undefined")return;window.localStorage.setItem(briefKey(brief.id),JSON.stringify(brief));const ids=loadBriefIds();if(!ids.includes(brief.id))writeIds(briefIndexKey,[brief.id,...ids].slice(0,200));}
