...
export const angleSynthesisSchema = {
  ...
  properties: {
    ...
    cases: { type:"array", minItems:3, maxItems:4, items:{ type:"object", additionalProperties:false, required:["key","company_name","case_title","case_summary","business_problem","tension","decision_or_move","mechanism","outcome","executive_implication","topic_relevance_score","campaign_alignment_score","relevance_score","credibility_score","tension_score","executive_value_score","brand_fit_score","confidence","sources"], properties:{
      key:{type:"string"}, company_name:{type:"string"}, case_title:{type:"string"}, case_summary:{type:"string"}, business_problem:{type:"string"}, tension:{type:"string"}, decision_or_move:{type:"string"}, mechanism:{type:"string"}, outcome:{type:"string"}, executive_implication:{type:"string"}, topic_relevance_score:score, campaign_alignment_score:score, relevance_score:score, credibility_score:score, tension_score:score, executive_value_score:score, brand_fit_score:score, confidence:{type:"string",enum:["low","medium","high"]}, sources:{type:"array",minItems:1,maxItems:6,items:{type:"object",additionalProperties:false,required:["ref","source_type","fact_notes"],properties:{ref:{type:"string"},source_type:{type:"string",enum:["official","government","media","research","academic","other"]},fact_notes:{type:"string"}}}}
    }}},
