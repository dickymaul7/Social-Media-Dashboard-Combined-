...
export const queryPlanSchema = {
  ...
  required: ["queries"],
  properties: { queries: { type: "array", minItems: 4, maxItems: 4, items: { type: "string", minLength: 1 } } },
} as const;

export const angleSynthesisSchema = {
  type: "object",
  additionalProperties: false,
  required: ["brand_profile", "campaign", "cases", "ideas"],
  properties: {
    brand_profile: {
      type: "object",
      additionalProperties: false,
      ...
      properties: {
        ...
        audience_pain_points: { type: "array", minItems: 2, maxItems: 5, items: { type: "string" } },
        tone_of_voice: { type: "string" },
        key_messages: { type: "array", minItems: 2, maxItems: 5, items: { type: "string" } },
        brand_pov: { type: "string" },
        core_expertise: { type: "array", minItems: 2, maxItems: 6, items: { type: "string" } },
        communication_dos: { type: "array", minItems: 2, maxItems: 5, items: { type: "string" } },
        communication_donts: { type: "array", minItems: 2, maxItems: 5, items: { type: "string" } },
      },
    },
    campaign: {
      type: "object",
      additionalProperties: false,
      required: ["desired_perception", "business_problem", "key_message", "funnel_stage"],
      properties: {
    ...
    cases: {
      ...
      items: {
        ...
        properties: {
          key: { type: "string" }, company_name: { type: "string" }, case_title: { type: "string" }, case_summary: { type: "string" }, business_problem: { type: "string" }, tension: { type: "string" }, decision_or_move: { type: "string" }, mechanism: { type: "string" }, outcome: { type: "string" }, executive_implication: { type: "string" }, relevance_score: score, credibility_score: score, tension_score: score, executive_value_score: score, brand_fit_score: score,
          confidence: { type: "string", enum: ["low", "medium", "high"] },
          sources: { type: "array", minItems: 1, maxItems: 6, items: { type: "object", additionalProperties: false, required: ["ref", "source_type", "fact_notes"], properties: { ref: { type: "string" }, source_type: { type: "string", enum: ["official", "government", "media", "research", "academic", "other"] }, fact_notes: { type: "string" } } } },
        },
      },
    },
    ideas: {
      type: "array", minItems: 1, maxItems: 10,
      items: { type: "object", additionalProperties: false, required: ["case_key", "working_title", "content_angle", "tension", "core_insight", "recommended_format", "campaign_relevance"], properties: { case_key: { type: "string" }, working_title: { type: "string" }, content_angle: { type: "string" }, tension: { type: "string" }, core_insight: { type: "string" }, recommended_format: { type: "string", enum: ["carousel", "reels", "single_post"] }, campaign_relevance: { type: "string" } } },
    },
  },
} as const;
